...
\bframe{Graphs, vertex coloring}{
	...
	\end{overprint}	
	\vskip 1 cm 
		\only<1-2>{\input{triangle}}\only<3>{\input{triangle_prop}} \qquad 
		\only<1-2>{\input{c5}}\only<3>{\input{c5_prop}} \qquad 
		\only<1-2>{\input{claw}}\only<3>{\input{claw_prop}}}
...
\bframe{Clique number, independence number}{
	...
	\end{itemize}
	\begin{center}\input{WAex}\end{center}}
...
\bframe{Chromatic number}{The \textit{chromatic number} of a graph $G$, denoted $\chi(G)$, is the minimum number of colors needed to properly color the vertices of $G$.
	\begin{overprint}	
		\onslide<1>\vskip 1 cm \input{triangle_prop} \qquad \input{c5_prop} \qquad \input{claw_prop}\\
...
\bframe{Connected matchings}{
 ...
 A \textit{connected matching} is a collection of disjoint edges such that each pair of edges has a pair of adjacent endpoints. \pause
	\begin{center}
	\begin{overprint}
		\onslide<3>\input{CMex_1}\\
		\onslide<4-> \input{CMex_1}\qquad\input{CMex_2}
	\end{overprint}
	\end{center}
}
...
\bframe{Definition}{
	...
	For all $u,v \in V(G)$, $i < k$
 	...
 	\end{enumerate}
	\vskip 0.5 cm
	\begin{center}\input{PPex_1}\pause\quad \input{PPex_2}\end{center}}
...
\bframe{Line graphs}{
	...
	\begin{center}\input{LGex_1}\pause\quad \input{LGex_2}\end{center}}
\bframe{Connected matching as a clique problem}{
	...
	\end{itemize}
	\begin{center}
		\uncover<3->{\input{coresp1}}\uncover<5->{\qquad\input{coresp2}}\uncover<7->{\qquad\input{coresp3}}
...
\bframe{Clique and clique-minor optimization}{
	...
	\begin{overprint}[6 cm]
		\onslide<2-3>\input{party1}
		\onslide<4>\input{party2}
	\end{overprint}
	\end{center}
}
\bframe{Clique and clique-minor optimization}{
	...
	\begin{center}	
	\begin{overprint}[6 cm]
		\onslide<1>\input{party2}
		\onslide<2>\input{party3}
	\end{overprint}
	\end{center}
}
...
\begin{center} \input{inclusion} \end{center}
}
